...
pp_freq_fish = zeros(length(freq_all),fish_num,length(z_mean));
% pp_freq = zeros(length(freq_all),1);
for iR=1:length(r_sigma)
    rr = randn(1,fish_num)*r_sigma(iR)+r_mean;
    zz = randn(1,fish_num)*z_sigma+z_mean;
    for iF=1:length(freq_all)
        ...
        if mode_incl~=0
            for iFISH=1:fish_num
                pp_freq_fish(iF,iFISH,iR) = get_pfield(modes,mode_incl,rr(iFISH),z0,zz(iFISH)).^2;  % roundtrip
            end
        end
    end
end
...
%         if mode_incl~=0
%             for iFISH=1:fish_num
%                 pp_freq_fish(iF,iFISH,iZ) = get_pfield(modes,mode_incl,rr(iFISH),z0,zz(iFISH)).^2;  % roundtrip
%             end
%         end
%     end
% end
