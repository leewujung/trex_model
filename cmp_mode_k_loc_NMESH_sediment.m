...
sediment_H = 5;
nmesh = [3000,5000,8000,11401,15000];
...
for iN=1:length(nmesh)
    ...
    fname = sprintf('%s_%dD_allModes_%ds.mod',fpre,sediment_H,nmesh(iN));
    ...
    k = modes.k;
    z = modes.z;
    save_fname = sprintf('%s_%dD_allModes_%ds_modeinfo.mat',fpre,sediment_H,nmesh(iN));
    save(fullfile(folder,save_fname),'k','phi','z');
end

figure(fig)
delta_mesh = 114./(nmesh-1);
