...
save_path = ['C:\Users\',usrn,'\Dropbox\0_APL_normal_mode\kraken\tests\wjlee_tests\20160406_ensemble'];
save_file = 'ensemble_test100.mat';
if ~exist(save_path,'dir')
    mkdir(save_path);
end

% env_path = 'F:\Dropbox\0_APL_normal_mode\kraken\tests\wjlee_tests\20160401_modes_calc';
env_path = ['C:\Users\',usrn,'\Dropbox\0_APL_normal_mode\kraken\tests\wjlee_tests\20160401_modes_calc'];
env_file_pre = 'mfenv';

freq_step = 1;
freq_all = 1:freq_step:2000;  % [Hz]
% param_path = 'F:\Dropbox\0_APL_normal_mode\kraken\tests\wjlee_tests\20160401_modes_calc';
param_path = ['C:\Users\',usrn,'\Dropbox\0_APL_normal_mode\kraken\tests\wjlee_tests\20160401_modes_calc'];
...
pp_freq = squeeze(nansum(pp_freq_fish_rep,2));

tt_vec = linspace(0,1/freq_step,size(pp_freq,1)*2+1);
...
tau = 1e-3;  % [sec]
s_len = size(pp_freq,1);
s_t = (-s_len:s_len)*tt_delta;
fc = 1000;
s = exp(-s_t.^2/tau^2).*(exp(1i*2*pi*fc*s_t)+exp(-1i*2*pi*fc*s_t))*1/2;
S = fft(s);
s_freq = linspace(0,1/tt_delta,length(S));
S_mtx = repmat(S.',1,rep_num);

% Multiplication in freq
tt_rz_mult = zeros(size(pp_freq,1)*2+1,length(z_mean),rep_num);
% tt_rz_mult_env = zeros(size(tt_rz,1),length(z_mean));
for iZ=1:length(z_mean)
    pp_freq_z = squeeze(pp_freq(:,iZ,:));
    pp_freq_z_cmplx = [zeros(1,size(pp_freq_z,2));pp_freq_z;flipud(conj(pp_freq_z))];
    tt_rz_mult_z = ifftshift(ifft(pp_freq_z_cmplx.*S_mtx));
    tt_rz_mult(:,iZ,:) = tt_rz_mult_z;
end
tt_vec_mult = linspace(0,1/freq_step,length(tt_rz_mult));

tt_rz_mult_energy_mean = mean(tt_rz_mult.^2,3);
